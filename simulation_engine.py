...
def _esegui_una_simulazione(params):
    ...
    for mese in range(1, mesi_totali + 1):
        ...
        rendimento_mese = np.random.lognormal(rendimento_medio_portfolio / 12 - 0.5 * (volatilita_portfolio / np.sqrt(12))**2, volatilita_portfolio / np.sqrt(12))
        stato['patrimonio_etf'] *= rendimento_mese

        if params['usa_fp'] and not stato['fp_liquidato']:
             rendimento_fp_mese = np.random.lognormal(params['fp_rendimento_netto'] / 12 - 0.5 * (0.05 / np.sqrt(12))**2, 0.05 / np.sqrt(12))
